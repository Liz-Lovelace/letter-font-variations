...
bot.on('message', async (msg) => {
  ...
  if (!allowedUsers.includes(userId)) {
    await bot.sendMessage(chatId, 'Uhhhhhh please talk to @Liz_Lovelace and tell her to add you to the allowedUsers list. Something is a bit broken :)');
    return;
  }
  ...
  if (msg.document) {
    ...
    await bot.sendMessage(chatId, 'Please send a file (that you exported from the figma file (the figma file with all the letters (the big frame in that file that is specifically for exporting it)))');
  }
});
